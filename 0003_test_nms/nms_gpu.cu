  ...
  if (threadIdx.x < y_threads) {
    ...
    for (int i = 0; i < x_threads; ++i) {
      ...
      if (iou(curbox, anobox) > thresh) {
        // printf ("box %d and %d iou: %f\n",row_start*blocksize+threadIdx.x, col_start*blocksize+i, iou(curbox,
        // anobox));
        tep |= (1U << i);
      }
    }
...
const vector<Scalar> COLORS = {Scalar(255, 0, 0),
                               Scalar(0, 255, 0),
                               Scalar(0, 0, 255),
                               Scalar(255, 255, 0),
                               Scalar(0, 255, 255),
...
int main(int argc, char *argv[]) {
  ...
  CUDA_CALL(
      cudaMemcpy(&mask_host[0], mask_dev, boxes.size() * blocknum * sizeof(unsigned int), cudaMemcpyDeviceToHost));
